...
| -------- | ---------- | ------------------------------------------------------------------ |
| Accepted | 2026-04-16 | [ADR-009_IMPLEMENTATION-PLAN.md](./ADR-009_IMPLEMENTATION-PLAN.md) |
...
Each template definition MUST provide `rootDir: import.meta.url` and typed metadata. The copied template assets MUST live in a sibling `files` directory:
...
A `defineTemplate(config)` helper will compute the internal `filesRoot` from `rootDir`. The computed `filesRoot` is runtime scaffolding data, not public metadata.
