...
| -------- | ---------- | ------------------------------------------------------------- |
| Accepted | 2026-04-15 | [ADR-005_IMPLEMENTATION-PLAN.md](./ADR-005_IMPLEMENTATION-PLAN.md) |
...
tsdown supports package validation through `publint` and `attw`. This package is currently CLI-only, and [ADR-004](../adr-004/ADR-004.md) says not to add `exports`, `main`, or `types` as part of the tsdown build setup.
...
```json
"release:prepare": "npm run test && npm run build -- --publint && npm pack --dry-run"
```
...
- Publishing takes longer because tests, build, package validation, and dry-run pack run before publish.
- Local npm cache ownership can break `npm pack`; this should be handled with a temporary npm cache, not a repository change.
...
- [ADR-004: Use tsdown for CLI builds](../adr-004/ADR-004.md)
