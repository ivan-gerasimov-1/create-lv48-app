...
4. Determine ADR number:
   ...
   - Normalize to three digits: `#14 Split decisions` -> `ADR-014`, `docs/adrs/ADR-014.md`.
   - Do not scan the input body for ADR numbers.
   - If no starting number exists, use max `ADR-NNN` from `docs/ADL.md` + 1.
5. Stop if `docs/adrs/ADR-NNN.md` already exists.
...
8. Create `docs/adrs/ADR-NNN.md` from `assets/ADR-TEMPLATE.md`.
